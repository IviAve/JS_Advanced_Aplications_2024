...
const endPoints = {
    addLike: '/data/likes',
    likesById: (albumId) => `/data/likes?where=albumId%3D%22${albumId}%22&distinct=_ownerId&count`,
    likesByUser: (albumId, userId) => `/data/likes?where=albumId%3D%22${albumId}%22%20and%20_ownerId%3D%22${userId}%22&count`
}
...
export async function getLikesByUser(albumId, userId) {
    return await get(endPoints.likesByUser(albumId, userId))
}